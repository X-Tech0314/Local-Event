    ...
    <div className="p-4 md:p-8 animate-fade-in relative min-h-[calc(100vh-4rem)]">
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded overflow-hidden p-2">
        <div className="relative">
          ...
          <div className="h-[420px] w-full rounded overflow-hidden relative border border-slate-200 dark:border-slate-700">
            <MapContainer
                ...
                >
                  ...
                  <div className="h-44 w-full relative overflow-hidden">
                    ...
                    {venue.isVerified && (
                      <div className="absolute top-3 right-3 z-20">
                        <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full bg-emerald-500/90 text-white">
                  ...
                  <div className="p-5 flex-grow flex flex-col">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      ...
                      <div className="bg-slate-50 dark:bg-slate-700/50 rounded p-2.5">
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Rating</p>
                        <p className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-1">
                          <span className="text-amber-500 text-xs">★</span> {venue.rating ? venue.rating.toFixed(1) : 'N/A'}
                        </p>
                      </div>
                      <div className="bg-slate-50 dark:bg-slate-700/50 rounded p-2.5">
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Floors</p>
                        <p className="text-sm font-bold text-slate-900 dark:text-white">{venue.numberOfFloors || 1}</p>
                      </div>
                    </div>

                    {/* Contact details */}
                    {(venue.contactPerson || venue.contactNumber || venue.contactEmail) && (
                      <div className="space-y-1.5 mb-4 pb-4 border-b border-slate-100 dark:border-slate-700">
                        {venue.contactPerson && (
                          <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">
                            ...
                            <span className="font-medium truncate">{venue.contactPerson}</span>
                          </div>
                        )}
                        {venue.contactNumber && (
                          <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">
                            <Phone size={11} className="text-purple-500 shrink-0" />
                            <span className="font-medium">{venue.contactNumber}</span>
                          </div>
                        )}
                        {venue.contactEmail && (
                          <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">
                            <Mail size={11} className="text-purple-500 shrink-0" />
                            <span className="font-medium truncate">{venue.contactEmail}</span>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    <div className="flex gap-2 mb-4 flex-wrap">
                      {venue.hasFireExit && (
                        <span className="flex items-center gap-1 text-[9px] font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-0.5 rounded-full">
                          <Shield size={9} /> Fire Exit
                        </span>
                      )}
                      {venue.hasFireExtinguishers && (
                        <span className="flex items-center gap-1 text-[9px] font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-0.5 rounded-full">
                          <Shield size={9} /> Extinguishers
                        </span>
                      )}
                      {venue.squareFootage > 0 && (
                        <span className="flex items-center gap-1 text-[9px] font-bold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded-full">
                          <Layers size={9} /> {venue.squareFootage.toLocaleString()} sqft
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-auto flex items-center justify-between gap-2">
                      {venue.mapUrl ? (
                        ...
                        <span className="text-xs text-slate-400 font-medium">No map link</span>
                      )}
                      <button
                        onClick={(e) => { e.stopPropagation(); setSelectedVenue(venue); }}
                        className="flex items-center gap-1.5 text-slate-800 dark:text-slate-200 font-bold text-xs px-3 py-1.5 bg-slate-100 dark:bg-slate-700 rounded hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                      >
