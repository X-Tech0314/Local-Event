...
import { MapPin, Plus, ExternalLink, Navigation, Building, AlertCircle } from 'lucide-react';
...
export default function VenuesPanel({ currentUser }) {
 ...
 const fetchVenues = async () => {
  ...
  try {
   ...
   // Fallback to mocks if backend is down
   const mockVenues = [
   ...
   setError('Could not connect to API. Showing mock data.');
  } finally {
   setLoading(false);
  }
 };
 ...
 />
 {venues.map((venue) => {
  const coords = venue.coords || [venue.latitude || 14.34, venue.longitude || 120.94];
  const title = venue.title || venue.name;
  const locationStr = venue.location || `${venue.barangay}, ${venue.city}`;
  return (
  ...
  <img src={venue.img || venue.imageUrl || 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=800&q=80'} className="w-full h-full object-cover" alt="Venue" />
  ...
  <div className="p-3">
  <h4 className="font-semibold text-slate-900 leading-tight mb-1 text-sm">{title}</h4>
  <p className="text-[11px] font-medium text-slate-500 mb-2 flex items-center gap-1"><MapPin size={10} /> {locationStr}</p>
  <div className="pt-2 border-t border-slate-100">
  <span className="text-[10px] font-semibold text-slate-400 ">Capacity: <span className="text-slate-800">{venue.capacity || 'N/A'}</span></span>
  </div>
  </div>
  </div>
  </Popup>
 ...
 {venues.map((venue) => {
  ...
  const locationStr = venue.location || `${venue.barangay}, ${venue.city}`;
  return (
  <div key={venue.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded overflow-hidden flex flex-col group hover:-translate-y-1 transition-all duration-300">
  
  <div className="h-40 w-full relative overflow-hidden">
  <div className="absolute inset-0 bg-slate-900/40 group-hover:bg-slate-900/20 transition-colors z-10"></div>
  <img src={venue.img || venue.imageUrl || 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=800&q=80'} alt={title} className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700" />
  ...
  <div className="mb-6">
  <h3 className="text-xl font-semibold text-slate-900 dark:text-white group-hover:text-slate-800 dark:text-slate-200 dark:group-hover:text-slate-500 transition-colors leading-tight mb-2 pr-12">{title}</h3>
  <p className="text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider">{locationStr}</p>
  </div>
  ...
  <p className="text-slate-900 dark:text-white font-semibold">{venue.capacity || 'N/A'}</p>
  </div>
