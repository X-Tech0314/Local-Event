    ...
    return (
        <div className="animate-fade-in space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black tracking-tight text-slate-900 dark:text-white">Attendees Hub</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">Monitor registrations and live check-in statuses.</p>
                </div>
                <button className="bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-900 dark:text-white px-6 py-3 rounded-xl font-bold transition-all shadow-sm flex items-center gap-2 active:scale-95">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    ...
                    <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm p-6 rounded-3xl relative overflow-hidden group hover:shadow-md transition-all duration-300">
                        <div className="flex justify-between items-start relative z-10">
                            <div>
                                <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">{stat.label}</p>
                                <p className="text-4xl font-black tracking-tight text-slate-900 dark:text-white">{stat.count}</p>
                            </div>
                            <div className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 group-hover:bg-purple-100 dark:group-hover:bg-purple-900/30 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">
                                <stat.icon size={24} strokeWidth={2.5} />
                            </div>
                        </div>
                        <p className={`text-xs font-bold mt-4 relative z-10 ${idx === 0 ? 'text-purple-600 dark:text-purple-400' : idx === 1 ? 'text-blue-500' : 'text-emerald-500'}`}>
                            {stat.sub}
                        </p>
                    </div>
                ))}
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
                ...
                <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex flex-col md:flex-row gap-4 justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                    <div className="relative w-full md:w-[400px]">
                        ...
                        <input type="text" placeholder="Search by name, email, or ticket ID..." className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl pl-12 pr-4 py-3 text-sm font-medium text-slate-900 dark:text-white focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all shadow-sm" />
                    </div>
                ...
                <div className="p-6 space-y-4">
                    {attendees.map((attendee, idx) => (
                        <div key={idx} className="flex flex-col lg:flex-row lg:items-center justify-between bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-purple-300 dark:hover:border-purple-600 hover:shadow-sm transition-all gap-6 group">
                            
                            {/* User Identity */}
                            <div className="flex items-center gap-4 min-w-[250px]">
                                <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${attendee.avatarColor} flex items-center justify-center text-white font-black text-lg shadow-sm border-2 border-white dark:border-slate-900`}>
                                    {attendee.name.split(' ').map(n => n[0]).join('')}
                                </div>
                                <div>
                                    <p className="font-black text-slate-900 dark:text-white leading-tight group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{attendee.name}</p>
                            ...
                            <div className="flex-1 lg:px-6 lg:border-x border-slate-200 dark:border-slate-700 flex flex-col justify-center">
                                <div className="flex items-center gap-2 mb-1">
                                    <Ticket size={14} className={attendee.ticket.includes('VIP') ? 'text-amber-500' : 'text-purple-600 dark:text-purple-400'} />
                                    <p className={`text-xs font-black uppercase tracking-widest ${attendee.ticket.includes('VIP') ? 'text-amber-500' : 'text-purple-600 dark:text-purple-400'}`}>
                                        {attendee.ticket}
                                    </p>
                                </div>
                            ...
                            <div className="flex items-center justify-between lg:justify-end gap-6 min-w-[200px]">
                                <div className="text-left lg:text-right">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-1">Arrival Time</p>
                                    <p className="text-sm font-bold text-slate-900 dark:text-white">{attendee.time}</p>
                                </div>
                                
                                {attendee.checkedIn ? (
                                    <div className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 px-4 py-2 rounded-xl">
                                        <CheckCircle2 size={18} strokeWidth={2.5} />
                                        <span className="text-xs font-black uppercase tracking-wider">Present</span>
                                    </div>
                                ) : (
                                    <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 px-4 py-2 rounded-xl">
                ...
                <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-between text-xs font-bold text-slate-500 dark:text-slate-400">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button className="px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg hover:border-slate-300 dark:hover:border-slate-600 text-slate-800 dark:text-white transition shadow-sm">Next</button>
                    </div>
                </div>
            </div>
        </div>
