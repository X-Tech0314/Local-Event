    ...
    if (showSuccess) {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center p-10 animate-fade-in text-center">
                <CheckCircle className="text-purple-500 w-24 h-24 mb-6 shadow-sm rounded-full bg-white dark:bg-slate-800" />
                <h2 className="text-4xl font-black text-slate-900 dark:text-white mb-4 tracking-tight">Event Published!</h2>
                <p className="text-slate-500 dark:text-slate-400 text-lg max-w-md mx-auto">Your event has been successfully published to the platform.</p>
                <div className="mt-8 flex items-center gap-4">
                    <button 
                        onClick={() => setActivePanel && setActivePanel('events')}
                        className="bg-purple-600 hover:bg-purple-700 text-white px-8 py-3 rounded-xl font-bold shadow-sm active:scale-95 transition-all"
                    >
                        View My Events
                    </button>
                    <button 
                    ...
                    <div className="max-w-2xl mx-auto w-full">
                        ...
                        <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-200 dark:border-slate-800 relative">
                            <div className="flex items-center gap-3 z-10">
                                ...
                                <span className={`font-bold text-xs tracking-wide transition-colors ${currentStep === 1 ? 'text-slate-900 dark:text-white' : currentStep > 1 ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}>Basic Info</span>
                            </div>
                            <div className={`h-px flex-1 mx-4 transition-colors duration-500 ${currentStep >= 2 ? 'bg-slate-900/50 dark:bg-white/50' : 'bg-slate-200 dark:bg-slate-700'}`}></div>
                            ...
                            <div className="flex items-center gap-3 z-10">
                                ...
                                <span className={`font-bold text-xs tracking-wide transition-colors ${currentStep === 2 ? 'text-slate-900 dark:text-white' : currentStep > 2 ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}>Logistics</span>
                            </div>
                            <div className={`h-px flex-1 mx-4 transition-colors duration-500 ${currentStep >= 3 ? 'bg-slate-900/50 dark:bg-white/50' : 'bg-slate-200 dark:bg-slate-700'}`}></div>
                            ...
                            <div className="flex items-center gap-3 z-10">
                                ...
                                <span className={`font-bold text-xs tracking-wide transition-colors ${currentStep === 3 ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}>Ticketing</span>
                            </div>
                        </div>
                        ...
                        {currentStep === 2 && (
                            <div className="space-y-6 animate-fade-in">
                                ...
                                <div>
                                    <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2"><MapPin size={16} className="text-purple-600 dark:text-purple-400" /> Location Details</h3>
                        ...
                        {currentStep === 3 && (
                            <div className="space-y-6 animate-fade-in">
                                ...
                                <div className="bg-slate-50 dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700">
                                    <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2"><Users size={16} className="text-purple-600 dark:text-purple-400" /> Ticketing Inventory Tiers</h3>
                                    ...
                                    <div className="space-y-3">
                                        <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Dynamic Allocation Tiers Matrix</label>
                                        
                                        {formData.TicketTiers.map((tier, index) => (
                                            <div key={index} className="flex flex-col gap-3 bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-700 relative group shadow-sm">
                                                <div className="flex gap-2 items-end">
                                                    <div className="flex-1">
                                                        ...
                                                        <input type="text" value={tier.TierName} onChange={(e) => updateTicketTier(index, 'TierName', e.target.value)} placeholder="e.g. VIP" className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-purple-500" />
                                                    </div>
                                                    <div className="w-24">
                                                        <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Slots</label>
                                                        <input type="number" value={tier.AllocatedSlots} onChange={(e) => updateTicketTier(index, 'AllocatedSlots', e.target.value)} placeholder="0" className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-purple-500" />
                                                    </div>
                                                    <div className="w-28 relative">
                                                        ...
                                                        {admissionStrategy === 'Free Admission' ? (
                                                            <input type="text" value="0.00" disabled className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700/60 rounded-lg px-2 py-2 text-sm text-slate-500 dark:text-slate-400 font-bold text-center cursor-not-allowed" />
                                                        ) : (
                                                            <div className="relative">
                                                                <span className="absolute left-2 top-2 text-slate-500 dark:text-slate-400 font-bold text-sm">₱</span>
                                                                <input type="number" step="0.01" value={tier.Price} onChange={(e) => updateTicketTier(index, 'Price', e.target.value)} placeholder="0.00" className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg pl-6 pr-2 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-purple-500" />
                                                            </div>
                                                        )}
                                                    </div>
                                                    <button onClick={() => removeTicketTier(index)} className="w-10 h-[38px] bg-red-50 dark:bg-red-900/30 text-red-500 hover:bg-red-500 hover:text-white rounded-lg transition-colors flex items-center justify-center border border-red-100 dark:border-red-800 hover:border-red-500">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                                <div className="w-full border-t border-slate-200 dark:border-slate-700 pt-2">
                                                    <label className="block text-[10px] font-bold text-purple-600 dark:text-purple-400 uppercase tracking-wider mb-1">Ticket Access Validity Scope</label>
                                                    <select 
                                                        value={tier.ValidityScope} onChange={(e) => updateTicketTier(index, 'ValidityScope', e.target.value)}
                                                        className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-purple-500"
                                                    >
                                ...
                                <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex gap-3">
                                    ...
                                    <button 
                                        ...
                                        className="flex-grow bg-purple-600 hover:bg-purple-700 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-[0.99] shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        {isSubmitting ? (
