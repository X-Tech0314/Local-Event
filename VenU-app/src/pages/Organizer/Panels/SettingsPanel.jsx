...
export default function SettingsPanel({ currentUser }) {
 ...
 const inputCls = "w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded px-4 py-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-purple-700 dark:border-purple-500 focus:ring-2 focus:ring-purple-500/20 focus:bg-white dark:focus:bg-slate-900 transition-all font-medium";
 const labelCls = "block text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-2";

 const Section = ({ title, children, className = "", icon: Icon }) => (
 <div className={`bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded p-8 mb-6 ${className}`}>
 <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2">
 {Icon && <div className="p-2 rounded bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 "><Icon size={18} strokeWidth={2.5} /></div>} {title}
 </h3>
 {children}
 </div>
 );
 ...
 <button onClick={onClick || handleSave} className="w-full md:w-auto px-8 py-3 rounded bg-purple-50 dark:bg-purple-900/30 hover:bg-purple-700 dark:bg-purple-500 hover:text-white text-purple-700 dark:text-purple-400 font-bold text-sm transition-all active:scale-95">
 {label}
 </button>
 </div>
 );
 ...
 {/* Header */}
 <div>
 <h1 className="text-3xl font-semibold text-slate-900 dark:text-white">Platform Settings</h1>
 <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">Configure your account preferences and security.</p>
 </div>

 {/* Premium Pill Tabs */}
 <div className="bg-white dark:bg-slate-800 p-2 rounded border border-slate-200 dark:border-slate-800 overflow-x-auto flex gap-2">
 {tabs.map(tab => (
 <button
 key={tab.id}
 onClick={() => setActiveTab(tab.id)}
 className={`px-6 py-3 rounded text-sm font-bold transition-all whitespace-nowrap flex-1 text-center ${activeTab === tab.id
 ? 'bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 '
 : 'bg-transparent text-slate-500 dark:text-slate-400 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 hover:bg-slate-50 dark:hover:bg-slate-800'
 }`}
 >
 {tab.label}
 </button>
 ...
 <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 text-center w-full">
 Profile Picture
 </label>
 <div className="relative group w-32 h-32">
 <div className="w-32 h-32 rounded bg-slate-200 dark:bg-slate-800 flex items-center justify-center font-semibold text-slate-400 dark:text-slate-600 text-4xl overflow-hidden border border-slate-300 dark:border-slate-700 transform transition-transform group-hover:scale-105">
 {profileImage ? (
 ...
 <label className="absolute inset-0 bg-slate-900/60 rounded flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity backdrop-blur-sm">
 ...
 <input type="email" value={form.email || ''} className="w-full bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded px-4 py-3 text-sm font-bold text-slate-400 dark:text-slate-600 cursor-not-allowed select-none" disabled />
 </div>
 </div>
 </div>
 ...
 <div onClick={() => oathRef.current.click()} className="border-2 border-dashed border-purple-700/50 dark:border-purple-500/50 dark:border-purple-700/50 dark:border-purple-500/50 rounded bg-slate-50/50 dark:bg-slate-800/50 p-10 flex flex-col items-center justify-center text-center cursor-pointer hover:border-purple-700 dark:border-purple-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all group">
 {oathDoc ? (
 ...
 <>
 <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full mb-4 group-hover:scale-110 transition-transform">
 ...
 <input type="text" value={form.idReferenceNumber || ''} onChange={set('idReferenceNumber')} placeholder="e.g. A000-1234-5678" className={`${inputCls} font-mono `} />
 </div>
 
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
 <div onClick={() => frontRef.current.click()} className="h-48 border-2 border-dashed border-purple-700/50 dark:border-purple-500/50 dark:border-purple-700/50 dark:border-purple-500/50 rounded bg-slate-50/50 dark:bg-slate-800/50 flex flex-col items-center justify-center text-center cursor-pointer hover:border-purple-700 dark:border-purple-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all overflow-hidden p-2">
 {idFront ? <img src={idFront} alt="Front ID" className="w-full h-full object-contain rounded" /> : <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Upload Front Side</p>}
 <input type="file" ref={frontRef} onChange={(e) => handleFileChange(e, setIdFront, 'idFrontPath')} className="hidden" accept="image/*" />
 </div>
 <div onClick={() => backRef.current.click()} className="h-48 border-2 border-dashed border-purple-700/50 dark:border-purple-500/50 dark:border-purple-700/50 dark:border-purple-500/50 rounded bg-slate-50/50 dark:bg-slate-800/50 flex flex-col items-center justify-center text-center cursor-pointer hover:border-purple-700 dark:border-purple-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all overflow-hidden p-2">
 {idBack ? <img src={idBack} alt="Back ID" className="w-full h-full object-contain rounded" /> : <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Upload Back Side</p>}
 <input type="file" ref={backRef} onChange={(e) => handleFileChange(e, setIdBack, 'idBackPath')} className="hidden" accept="image/*" />
 </div>
 </div>
 
 <div className="mb-6">
 <div onClick={() => selfieRef.current.click()} className="h-48 border-2 border-dashed border-purple-700/50 dark:border-purple-500/50 dark:border-purple-700/50 dark:border-purple-500/50 rounded bg-slate-50/50 dark:bg-slate-800/50 flex flex-col items-center justify-center text-center cursor-pointer hover:border-purple-700 dark:border-purple-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all overflow-hidden p-2">
 {idSelfie ? <img src={idSelfie} alt="Selfie" className="w-full h-full object-contain rounded" /> : <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Upload Selfie with ID</p>}
 ...
 {wallets.length === 0 ? (
 <div className="text-center py-12 px-6 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded">
 ...
 <button onClick={() => setShowAddWalletModal(true)} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 font-bold px-8 py-3 rounded hover:border-purple-700 dark:border-purple-500 transition-all active:scale-95">
 + Add Payout Method
 </button>
 </div>
 ) : (
 ...
 {wallets.map(wallet => (
 <div key={wallet.id} className="border border-slate-200 dark:border-slate-700 rounded p-5 flex items-center justify-between hover:border-purple-700 dark:border-purple-500 hover: transition-all group bg-white dark:bg-slate-800">
 <div className="flex items-center gap-4">
 <div className={`w-12 h-12 rounded ${wallet.bg} flex items-center justify-center font-bold text-lg ${wallet.text} group-hover:scale-110 transition-transform`}>
 {wallet.icon}
 </div>
 <div>
 ...
 <button onClick={() => setShowAddWalletModal(true)} className="w-full py-4 border-2 border-dashed border-purple-700/50 dark:border-purple-500/50 dark:border-purple-700/50 dark:border-purple-500/50 rounded text-sm font-semibold text-purple-700 dark:text-purple-400 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:border-purple-700 dark:border-purple-500 transition-all">
 + Add Another Method
 </button>
 </div>
 )}
 ...
 <div className="animate-fade-in space-y-6">
 {/* High-Notice Banner */}
 <div className="bg-purple-50 dark:bg-purple-900/30 border border-purple-100 dark:border-purple-800/50 rounded p-8 relative overflow-hidden">
 <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
 <div className="p-4 bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-400 rounded">
 <Bell size={32} strokeWidth={2.5} />
 </div>
 <div className="text-center md:text-left">
 <h2 className="text-2xl font-bold mb-2 text-purple-900 dark:text-white">Notifications Enabled</h2>
 <p className="text-sm font-medium text-purple-700 dark:text-purple-400 max-w-lg">Customize how you receive system alerts, payouts, and updates.</p>
 </div>
 </div>
 </div>
 ...
 <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700 mb-6">
 <ToggleSwitch
 ...
 <div className="p-6 border border-slate-200 dark:border-slate-800 rounded">
 <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-2">Active Sessions</p>
 <p className="text-sm font-bold text-slate-900 dark:text-white mb-4">Current Session: <span className="text-slate-800 dark:text-slate-200 ">Windows (Browser)</span></p>
 <button onClick={() => alert("Successfully signed out of all other sessions")} className="px-6 py-2 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-sm hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors active:scale-95">
 ...
 <div className="bg-slate-900 dark:bg-slate-900 text-white p-6 rounded mb-6 relative overflow-hidden">
 ...
 <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-4 bg-slate-50 dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700">
 ...
 <button onClick={handlePrintTranscript} className="border-2 border-purple-600 text-slate-800 dark:text-slate-200 font-bold text-xs px-6 py-3 rounded hover:bg-slate-100 dark:hover:bg-purple-900/10 transition-all active:scale-95 whitespace-nowrap">
 Export Data
 </button>
 </div>
 </Section>
 ...
 <div className="mt-12 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded p-8 relative overflow-hidden group">
 <div className="flex items-start gap-4 relative z-10">
 <div className="p-3 bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400 rounded">
 ...
 <button onClick={() => setShowDangerModal(true)} className="bg-red-600 hover:bg-red-700 text-white font-bold text-sm px-8 py-3 rounded transition-all active:scale-95">
 Delete Account
 </button>
 </div>
 </div>
 ...
 <div className="bg-white dark:bg-slate-800 rounded w-full max-w-md overflow-hidden p-8 text-center animate-scale-in border border-slate-200 dark:border-slate-800">
 ...
 <button onClick={() => setShowDangerModal(false)} className="flex-1 py-3.5 rounded border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-all active:scale-95">Cancel</button>
 <button onClick={() => alert("Account Deletion Initiated.")} className="flex-1 py-3.5 rounded bg-red-600 hover:bg-red-700 text-white font-bold text-sm transition-all active:scale-95">Confirm Delete</button>
 </div>
 </div>
 </div>
 )}
 ...
 {walletToUnlink && (
 <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4">
 <div className="bg-white dark:bg-slate-800 rounded w-full max-w-sm overflow-hidden p-8 text-center animate-scale-in border border-slate-200 dark:border-slate-800">
 <div className="w-16 h-16 rounded bg-red-50 dark:bg-red-900/30 flex items-center justify-center mx-auto mb-6 border border-red-100 dark:border-red-900/50 text-red-500 dark:text-red-400">
 ...
 <button onClick={() => setWalletToUnlink(null)} className="flex-1 py-3 rounded border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-all active:scale-95">Cancel</button>
 <button onClick={() => { setWallets(wallets.filter(w => w.id !== walletToUnlink.id)); setWalletToUnlink(null); }} className="flex-1 py-3 rounded bg-red-500 hover:bg-red-600 text-white font-bold text-sm transition-all active:scale-95">Unlink</button>
 </div>
 </div>
 </div>
 )}
 ...
 {showAddWalletModal && (
 <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4">
 <div className="bg-white dark:bg-slate-800 rounded w-full max-w-lg overflow-hidden p-8 animate-scale-in border border-slate-200 dark:border-slate-800">
