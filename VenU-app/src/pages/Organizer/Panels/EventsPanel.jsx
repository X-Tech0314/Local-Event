...
export default function EventsPanel({ currentUser, setActivePanel }) {
 ...
 <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-slate-600 dark:text-slate-400 transition-colors" size={18} />
 ...
 <div className="w-10 h-10 border-4 border-purple-700 dark:border-purple-500 border-t-transparent rounded-full animate-spin"></div>
 </div>
 ) : error ? (
 ...
 return (
 <div key={evt.id} className="bg-white dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-800 overflow-hidden hover: hover:-translate-y-1 transition-all duration-300 group flex flex-col relative">
 {/* Event Thumbnail */}
 ...
 <span className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-semibold border backdrop-blur-md bg-white/90 ${status === 'Active' ? 'text-slate-700 dark:text-slate-300 border-emerald-500/20' : 'text-slate-500 border-slate-200'}`}>
 ...
 {microLocation && (
 <span className="text-[10px] text-slate-800 dark:text-slate-200 font-semibold mt-0.5">{microLocation}</span>
 )}
 {evt.mapUrl && (
 <a 
 href={evt.mapUrl} 
 ...
 <div className="mt-3 text-[10px] font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1.5 bg-slate-700 dark:bg-slate-300/5 border border-purple-500/10 px-2.5 py-1.5 rounded-lg">
 <span className="w-1.5 h-1.5 rounded-full bg-slate-700 dark:bg-slate-300 animate-pulse"></span>
 ...
 <p className="text-[9px] font-mono font-bold text-slate-400 dark:text-slate-500 ">ID: {evt.id.substring(0, 8)}</p>
