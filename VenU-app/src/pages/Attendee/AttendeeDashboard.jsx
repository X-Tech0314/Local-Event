...
// ── Organizer's 7 standard event categories ─────────────────────────────────
const STANDARD_CATEGORIES = [
  'Tech & Innovation', 'Music & Concerts', 'Sports & Athletics',
  'Business & Corp', 'Birthdays', 'Weddings', 'Others'
];
...
const CATEGORY_ALIASES = {
  ...
  'business & corp': 'Business & Corp',
  'business & corporate': 'Business & Corp',
  'birthday': 'Birthdays',
  'birthdays': 'Birthdays',
  'wedding': 'Weddings',
  'weddings': 'Weddings',
...
function normalizeCategory(cat) {
  ...
  if (key.includes('music') || key.includes('concert')) return 'Music & Concerts';
  if (key.includes('tech') || key.includes('innov')) return 'Tech & Innovation';
  if (key.includes('sport') || key.includes('athlet')) return 'Sports & Athletics';
  if (key.includes('business') || key.includes('corp')) return 'Business & Corp';
  if (key.includes('birthday') || key.includes('birth')) return 'Birthdays';
  if (key.includes('wedding')) return 'Weddings';
  return 'Others';
}
...
const mockEvents = mockEventsRaw.map((e) => {
  const normalized = normalizeCategory(e.category);
  return {
    ...e,
    category: normalized,
    color: CATEGORY_GRADIENTS[normalized] || e.color || CATEGORY_GRADIENTS['Others'],
  };
});
...
function mapBackendEvent(evt) {
  ...
  const normalized = normalizeCategory(evt.category);

  return {
    id: evt.id,
    title: evt.title || 'Untitled Event',
    category: normalized,
    barangay: locationLabel,
    city: evt.city || '',
    date: dateStr,
    time: timeStr,
    ...
    color: CATEGORY_GRADIENTS[normalized] || CATEGORY_GRADIENTS['Others'],
    status: evt.status,
...
function getRecommendedEvents(events, user) {
  ...
  return { events: sortedEvents, scope: scopeTitle };
}
  ...
  const handleCheckout = () => {
    ...
    onSuccess({
      event: {
        ...event,
        category: normalized,
        color: CATEGORY_GRADIENTS[normalized] || event.color || CATEGORY_GRADIENTS['Others'],
      },
    ...
    <div className="fixed inset-0 z-[70] flex justify-end">
      ...
      <div className="w-full max-w-md bg-slate-50 dark:bg-slate-900 h-full shadow-2xl border-l border-slate-200 dark:border-slate-800 flex flex-col animate-slide-in-right relative z-10 overflow-hidden">
          ...
          {event.accessType === 'Private' && (
            <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-none border border-slate-200 dark:border-slate-700">
              ...
              <div className="relative">
                <input type="text" value={accessCode} onChange={(e) => setAccessCode(e.target.value.toUpperCase())} placeholder="ENTER ACCESS CODE"
                  className={`w-full bg-slate-50 dark:bg-slate-900 border-2 rounded-none px-4 py-4 text-center font-bold tracking-[0.1em] uppercase text-slate-900 dark:text-white outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500 ${accessCode && !codeMatches ? 'border-red-400 focus:border-red-500' : codeMatches ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400' : 'border-slate-200 dark:border-slate-700 focus:border-purple-500'}`}
                />
              </div>
              {accessCode && !codeMatches && (<p className="text-xs text-red-500 mt-3 font-semibold text-center">Invalid Pass Code.</p>)}
              {codeMatches && (<p className="text-xs text-emerald-600 dark:text-emerald-400 mt-3 font-semibold text-center">Code verified. You're on the list.</p>)}
            </div>
          )}
          {event.isPaid && (
            <div>
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-none p-5">
                <div className="flex justify-between text-sm text-slate-600 dark:text-slate-400 mb-3 font-medium"><span>Subtotal</span><span>₱{event.price.toLocaleString()}</span></div>
                <div className="flex justify-between text-sm text-slate-600 dark:text-slate-400 mb-4 font-medium"><span>Platform Fee</span><span>₱0.00</span></div>
        ...
        <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md relative z-20">
          {event.accessType === 'Private' ? (
            <button disabled={!codeMatches} onClick={handleCheckout}
              className={`w-full py-4 rounded-none text-sm font-bold uppercase tracking-wide ${codeMatches ? 'bg-purple-600 hover:bg-purple-700 text-white shadow-sm active:scale-95' : 'bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-600 cursor-not-allowed'}`}>
      ...
      <div className="w-full max-w-sm animate-scale-in relative group perspective-1000">
        <div className="bg-white rounded-none overflow-hidden shadow-2xl relative z-10 transform transition-transform preserve-3d">
          <div className={`bg-gradient-to-br ${ticket.event.color || 'from-purple-500 to-indigo-600'} p-6 pb-12 text-white text-center relative`}>
            <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full bg-black/20 hover:bg-black/40 backdrop-blur-md"><X size={14} strokeWidth={3} /></button>
          ...
          <div className="p-8 pt-6 flex flex-col items-center bg-white relative">
            <div className="w-40 h-40 bg-white border border-slate-200 shadow-sm rounded-none p-4 mb-6 relative overflow-hidden">
              ...
              <div className="relative z-10 w-full h-full"><QrMockup /></div>
            </div>
            <div className="text-center w-full mb-6">
              <p className="text-[10px] text-slate-400 uppercase tracking-widest font-black mb-1">Ticket ID</p>
              <div className="bg-slate-50 border border-slate-100 rounded-none py-2 px-4 font-mono font-black text-slate-900 tracking-widest text-sm shadow-inner">{ticket.ticketId}</div>
            </div>
            <div className="w-full grid grid-cols-2 gap-y-4 gap-x-6 text-left">
              <div><p className="text-[10px] text-slate-400 uppercase tracking-widest font-black mb-0.5">Tier</p><p className="font-bold text-slate-900 leading-tight text-sm">{ticket.tier}</p></div>
              <div><p className="text-[10px] text-slate-400 uppercase tracking-widest font-black mb-0.5">Date</p><p className="font-bold text-slate-900 leading-tight text-sm">{ticket.event.date}</p></div>
              <div className="col-span-2"><p className="text-[10px] text-slate-400 uppercase tracking-widest font-black mb-0.5">Venue</p><p className="font-bold text-slate-900 leading-tight text-sm">{ticket.event.barangay}</p></div>
            </div>
          </div>
          <div className="bg-slate-50 border-t border-slate-100 p-4 text-center">
            <button onClick={onClose} className="text-xs font-black uppercase tracking-widest text-purple-700 dark:text-purple-500 hover:text-purple-900">Close Pass</button>
          </div>
        </div>
      </div>
    </div>
...
export default function AttendeeDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const visible = rawEvents.map(mapBackendEvent).filter(evt => evt && (!evt.status || evt.status === 'Published'));
        if (visible.length > 0) { setEvents(visible); } else { setEvents(mockEvents); }
      } catch (err) {
        if (cancelled) return;
  ...
  const [tickets, setTickets] = useState(() => {
    ...
    const migrated = parsed.map(t => ({
      ...t,
      event: t.event ? { ...t.event, category: normalizeCategory(t.event.category) } : t.event,
    }));
    localStorage.setItem('vnu_user_tickets', JSON.stringify(migrated));
    return migrated;
  });
  ...
  const filteredTickets = tickets.filter((t) => {
    const ticketCategory = normalizeCategory(t.event?.category);
    const matchesCategory =
      selectedWalletCategory === 'All' ||
      ticketCategory.toLowerCase() === selectedWalletCategory.toLowerCase();

    const q = walletSearch.toLowerCase().trim();
    const matchesSearch =
      !q ||
      t.event?.title?.toLowerCase().includes(q) ||
      ticketCategory.toLowerCase().includes(q) ||
      t.event?.barangay?.toLowerCase().includes(q) ||
  ...
  const handleDownloadTicket = (ticket, e) => {
    ...
    if (gradientColor.includes('emerald')) { cssGradient = 'linear-gradient(135deg, #10b981, #0d9488)'; }
    else if (gradientColor.includes('amber')) { cssGradient = 'linear-gradient(135deg, #f59e0b, #ea580c)'; }
    else if (gradientColor.includes('blue')) { cssGradient = 'linear-gradient(135deg, #3b82f6, #06b6d4)'; }
    else if (gradientColor.includes('pink') || gradientColor.includes('rose')) { cssGradient = 'linear-gradient(135deg, #ec4899, #e11d48)'; }
    else if (gradientColor.includes('slate')) { cssGradient = 'linear-gradient(135deg, #64748b, #334155)'; }

    const container = document.createElement('div');
    container.style.position = 'fixed'; container.style.top = '-9999px'; container.style.left = '-9999px';
    container.style.width = '384px'; container.style.fontFamily = '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
    container.style.backgroundColor = '#0f172a'; container.style.padding = '40px 20px';
    const qrPixels = Array.from({ length: 64 }, () => `<div style="background-color: ${Math.random() > 0.5 ? '#0f172a' : 'transparent'}; width:100%; height:100%;"></div>`).join('');
    container.innerHTML = `
      <div style="width: 100%; background: white; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); overflow: hidden; position: relative;">
        ...
        <div style="padding: 32px; padding-top: 24px; text-align: center;">
          <div style="width: 160px; height: 160px; border: 1px solid #e2e8f0; padding: 16px; margin: 0 auto 24px auto; display: grid; grid-template-columns: repeat(8, 1fr); gap: 2px; background: #fff;">${qrPixels}</div>
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; row-gap: 16px; column-gap: 24px; text-align: left;">
            <div><div style="font-size: 10px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.15em; font-weight: 900; margin-bottom: 4px;">Tier</div><p style="font-weight: 700; color: #0f172a; font-size: 14px; margin: 0; line-height: 1.2;">${ticket.tier}</p></div>
            <div><div style="font-size: 10px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.15em; font-weight: 900; margin-bottom: 4px;">Date</div><p style="font-weight: 700; color: #0f172a; font-size: 14px; margin: 0; line-height: 1.2;">${ticket.event.date}</p></div>
            <div style="grid-column: span 2;"><div style="font-size: 10px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.15em; font-weight: 900; margin-bottom: 4px;">Venue</div><p style="font-weight: 700; color: #0f172a; font-size: 14px; margin: 0; line-height: 1.2;">${ticket.event.barangay}</p></div>
          </div>
        </div>
    ...
    html2canvas(container, { backgroundColor: '#0f172a', scale: 2, logging: false, useCORS: true }).then((canvas) => {
      const link = document.createElement('a');
      link.download = `VenU-Pass-${ticket.ticketId}.png`;
      link.href = canvas.toDataURL('image/png');
      link.click();
      document.body.removeChild(container);
    }).catch((err) => { console.error("Error drawing passport asset:", err); document.body.removeChild(container); });
  };
  ...
  return (
    <div className="flex min-h-screen bg-slate-200 dark:bg-slate-900 font-sans text-slate-900 dark:text-slate-100">
      <aside className="w-64 bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 h-screen fixed left-0 top-0 p-6 flex flex-col justify-between z-40 shadow-2xl">
        <div className="flex flex-col gap-8">
          ...
          <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3 rounded">
            <div className="w-10 h-10 rounded-full bg-purple-700 dark:bg-purple-500 flex items-center justify-center font-medium text-white text-sm shrink-0">{currentUser.firstName.charAt(0)}</div>
            ...
            {navItems.map(({ id, label, icon: Icon }) => {
              ...
              return (
                <button key={id} onClick={() => setActiveTab(id)}
                  ...
                  {id === 'mytickets' && tickets.length > 0 && (<span className="ml-auto bg-purple-200 dark:bg-purple-900 text-purple-800 dark:text-purple-300 text-xs font-black px-2 py-0.5 rounded-full">{tickets.length}</span>)}
                </button>
              );
            })}
          </nav>
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <button onClick={() => setShowNotifications(!showNotifications)}
                ...
                {unreadCount > 0 && (<span className="absolute top-2 right-2 flex h-2.5 w-2.5"><span className="animate-ping absolute inline-flex h-full w-full rounded-none bg-purple-400 opacity-75"></span><span className="relative inline-flex rounded-none h-2.5 w-2.5 bg-purple-700 dark:bg-purple-500 border-2 border-white dark:border-slate-800"></span></span>)}
              </button>
              {showNotifications && (
                <div className="absolute top-full right-0 mt-4 w-[360px] bg-slate-50/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-none shadow-2xl border border-slate-100/50 dark:border-slate-700/50 z-50 overflow-hidden animate-fade-in origin-top-right">
                  <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                    <div className="flex items-center gap-2"><h3 className="text-sm font-black text-slate-900 dark:text-white tracking-tight">Notifications</h3>{unreadCount > 0 && (<span className="bg-purple-700 dark:bg-purple-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{unreadCount} New</span>)}</div>
                    {unreadCount > 0 && (<button onClick={markAllRead} className="text-xs font-bold text-purple-700 dark:text-purple-500 hover:text-purple-700">Mark all read</button>)}
                  </div>
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.length === 0 ? (<div className="p-8 text-center"><div className="w-12 h-12 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center mx-auto mb-3 text-slate-300"><Bell size={20} /></div><p className="text-sm font-bold text-slate-400">All caught up!</p></div>) : (notifications.map(notif => (
                      <div key={notif.id} className={`p-5 border-b border-slate-50 last:border-0 hover:bg-slate-50/80 cursor-pointer relative group ${notif.read ? 'opacity-60' : ''}`}>
                        {!notif.read && (<div className="absolute left-0 top-0 bottom-0 w-1 bg-purple-700 dark:bg-purple-500 rounded-r-md"></div>)}
                        <div className="flex justify-between items-start mb-1.5 pl-2"><h4 className={`text-sm font-black ${notif.read ? 'text-slate-600' : 'text-slate-900'}`}>{notif.title}</h4><span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{notif.time}</span></div>
            ...
            <button onClick={toggleDarkMode} className="p-2.5 rounded-none bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600 hover:text-slate-800 dark:hover:text-white" title={darkMode ? 'Switch to Light Mode' : 'Switch to Dark Mode'}>
        ...
        <div className="p-8 max-w-7xl mx-auto w-full">
          {activeTab === 'dashboard' && (
            <div className="animate-fade-in space-y-12">
              <div className="bg-slate-50 dark:bg-slate-800 rounded-none p-10 relative overflow-hidden shadow-md border border-slate-200 dark:border-slate-700">
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                  <div className="text-center md:text-left">
                    <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-widest font-bold mb-2 flex items-center gap-2 justify-center md:justify-start"><SparklesIcon /> Explore Events</p>
                    <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tight leading-tight">Find your next <span className="text-slate-700 dark:text-slate-300">experience.</span></h1>
                  </div>
                  <div className="w-full md:w-[450px] relative group">
                    <div className="relative bg-slate-50 dark:bg-slate-700/50 backdrop-blur-md border border-slate-200 dark:border-slate-600 rounded-none p-2 flex items-center shadow-lg">
                      <div className="p-3 text-slate-400 dark:text-slate-500"><Search size={20} strokeWidth={2.5} /></div>
                      <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search events, venues, access codes..." className="w-full bg-transparent text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 font-medium text-sm outline-none px-2 pr-4" />
                      <button className="bg-white text-slate-900 px-4 py-2.5 rounded-none text-xs font-black uppercase tracking-widest hover:bg-slate-100 shadow-sm">Scan</button>
                    </div>
                  </div>
                </div>
              </div>

              {/* Connected Discovery Map Dashboard Element */}
              {filtered.length > 0 && (
                <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-2 shadow-sm rounded-none">
                  <div className="h-[300px] w-full rounded-none overflow-hidden relative border border-slate-200 dark:border-slate-700">
                    <MapContainer center={[14.3296, 120.9367]} zoom={11} scrollWheelZoom={false} style={{ height: '100%', width: '100%' }}>
                      <TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a>' url="https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png" />
                      {filtered.map((event, i) => (
                        <Marker key={event.id} position={event.city === 'Imus' ? [14.4296, 120.9367] : [14.3296 + (i * 0.01), 120.9367 - (i * 0.01)]}>
                          <Popup className="rounded-none overflow-hidden shadow-2xl border-0 p-0 m-0 w-[240px]">
                            <div className="font-sans" onClick={() => setSelectedEvent(event)}>
                              <div className="h-24 w-full bg-slate-200 relative cursor-pointer">
                              ...
                              <div className="p-3 cursor-pointer">
                                <h4 className="font-black text-slate-900 leading-tight mb-1 text-sm hover:text-purple-700">{event.title}</h4>
                                <p className="text-[11px] font-medium text-slate-500 flex items-center gap-1"><MapPin size={10} /> {event.barangay}</p>
                              </div>
                            </div>
                          </Popup>
                        </Marker>
              ...
              <div>
                <div className="flex items-center justify-between mb-8 px-2">
                  <div><h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-purple-700 dark:bg-purple-500 animate-pulse"></div> {scope}</h2></div>
                  <span className="text-xs font-bold uppercase tracking-wide text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800 px-4 py-1.5 rounded-full shadow-sm">{filtered.length} event{filtered.length !== 1 ? 's' : ''} found</span>
                </div>
                {eventsLoading ? (
                  ...
                  <div className="text-center py-32 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-none shadow-sm relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                      <div className="w-24 h-24 bg-slate-50 dark:bg-slate-900 shadow-sm border border-slate-200 dark:border-slate-700 rounded-full flex items-center justify-center mx-auto mb-6 text-slate-300 dark:text-slate-500"><Search size={32} strokeWidth={2.5} /></div>
                      ...
                      {eventsError && (<p className="text-xs text-amber-500 mt-3 font-medium">Showing sample events (organizer API unreachable: {eventsError}).</p>)}
                    </div>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    {filtered.map((event) => (<EventCard key={event.id} event={event} onSelect={setSelectedEvent} />))}
                  </div>
                )}
              </div>
            </div>
          ...
          {activeTab === 'mytickets' && (
            <div className="animate-fade-in max-w-6xl mx-auto">
              <div className="mb-6 text-center">
                <div className="w-16 h-16 bg-purple-100 dark:bg-purple-900/30 rounded-none flex items-center justify-center mx-auto mb-4 text-purple-600 dark:text-purple-400"><Ticket size={28} strokeWidth={2.5} /></div>
              ...
              <div className="mb-4 relative">
                ...
                <input type="text" value={walletSearch} onChange={(e) => setWalletSearch(e.target.value)} placeholder="Search your tickets by name, venue, or tier..." className="w-full pl-10 pr-10 py-3 text-sm font-medium bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-none focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent placeholder:text-slate-400" />
                {walletSearch && (<button onClick={() => setWalletSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"><X size={16} /></button>)}
              </div>
              <div className="mb-10 bg-slate-50 dark:bg-slate-800/40 p-4 border border-slate-200 dark:border-slate-800 rounded-none">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  {eventTypes.map((type) => {
                    ...
                    return (
                      <button key={type.name} onClick={() => setSelectedWalletCategory(type.name)}
                        className={`flex items-center gap-3 p-3 text-sm font-bold border transition-all ${isSelected ? 'bg-purple-700 dark:bg-purple-600 border-purple-600 text-white shadow-md' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-700'}`}>
              ...
              {filteredTickets.length === 0 ? (
                <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm rounded-none p-16 text-center relative overflow-hidden group">
                  ...
                  <p className="text-sm font-medium text-slate-400 mt-2 mb-8 max-w-sm mx-auto">{tickets.length === 0 ? "You haven't purchased any tickets yet. Browse events to find something you like." : walletSearch ? "No passes match your search. Try a different keyword." : "No passes match this specific event type filter."}</p>
                  {tickets.length === 0 && (<button onClick={() => setActiveTab('dashboard')} className="px-8 py-3.5 rounded-none bg-purple-600 hover:bg-purple-700 text-white text-sm font-bold shadow-sm active:scale-95 flex items-center gap-2 mx-auto"><Search size={16} /> Explore Events</button>)}
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {filteredTickets.map((t) => (
                    <div key={t.ticketId} onClick={() => setConfirmedTicket(t)} className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-none overflow-hidden relative group cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col justify-between">
                      <div className="p-5 flex gap-4">
                        <div className="w-20 h-20 bg-slate-200 dark:bg-slate-900 shrink-0 border border-slate-200 dark:border-slate-700 overflow-hidden"><img src={t.event.image} alt="" className="w-full h-full object-cover" /></div>
                        <div className="min-w-0 flex-1">
                          <span className="text-[9px] font-black tracking-widest text-purple-600 dark:text-purple-400 uppercase bg-purple-50 dark:bg-purple-900/30 px-2 py-0.5 border border-purple-100 dark:border-purple-800/50">{t.tier}</span>
                          <h3 className="font-black text-slate-900 dark:text-white mt-1.5 truncate text-base leading-tight">{t.event.title}</h3>
                      ...
                      <div className="bg-slate-100 dark:bg-slate-900 px-5 py-3 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                        <span className="font-mono text-xs font-black text-slate-700 dark:text-slate-300 tracking-wider">{t.ticketId}</span>
                        <div className="flex items-center gap-1.5">
                          <button title="View Full Digital Pass" onClick={() => setConfirmedTicket(t)} className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-purple-500 dark:hover:border-purple-500 text-slate-600 dark:text-slate-300 hover:text-purple-600 dark:hover:text-purple-400 transition-colors"><Eye size={14} strokeWidth={2.5} /></button>
                          <button title="Download Ticket Image (.png)" onClick={(e) => handleDownloadTicket(t, e)} className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-emerald-500 dark:hover:border-emerald-500 text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors"><Download size={14} strokeWidth={2.5} /></button>
                          <button title="Void / Delete Pass" onClick={(e) => handleDeleteTicket(t.ticketId, t.event.title, e)} className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-red-500 dark:hover:border-red-500 text-slate-600 dark:text-slate-300 hover:text-red-500 dark:hover:text-red-400 transition-colors"><Trash2 size={14} strokeWidth={2.5} /></button>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {activeTab === 'map' && (
            <div className="animate-fade-in">
              <div className="flex items-center gap-4 mb-8">
                <div className="p-3 bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 rounded-none"><MapPin size={24} strokeWidth={2.5} /></div>
                <div><h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Locations</h1><p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">Explore events happening around you on the map.</p></div>
              </div>
              <div className="relative">
                        ...
                        <Marker key={event.id} position={event.city === 'Imus' ? [14.4296, 120.9367] : [14.3296 + (i * 0.01), 120.9367 - (i * 0.01)]}>
                          <Popup className="rounded-none overflow-hidden shadow-2xl border-0 p-0 m-0 w-[240px]">
                            <div className="font-sans">
                              <div className="h-24 w-full bg-slate-200 relative"><img src={event.image} className="w-full h-full object-cover" alt="Venue" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div><span className="absolute bottom-2 left-2 text-[10px] font-black uppercase tracking-widest text-white px-2 py-1 bg-purple-700 dark:bg-purple-500/80 rounded-none backdrop-blur-sm">{event.category}</span></div>
                              <div className="p-3"><h4 className="font-black text-slate-900 leading-tight mb-1 text-sm">{event.title}</h4><p className="text-[11px] font-medium text-slate-500 mb-2 flex items-center gap-1"><MapPin size={10} /> {event.barangay}</p></div>
                            </div>
                          </Popup>
                        </Marker>
                      ))}
      ...
      {selectedEvent && (<TicketingDrawer event={selectedEvent} onClose={() => setSelectedEvent(null)} onSuccess={handleSuccess} />)}
      {confirmedTicket && (<TicketModal ticket={confirmedTicket} onClose={() => setConfirmedTicket(null)} />)}
    </div>
  );
}